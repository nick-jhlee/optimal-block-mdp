...
for i, u in enumerate(us):
    ...
    sns.stripplot(x="n", y="error rate", hue="Algorithm", data=mdf, dodge=True, ax=ax, legend=False)
    fig.suptitle(f"Experiment 0: Varying n (TH = n (log n)^{u}, eps={eps})")
    plt.savefig(f"results/plot_exp0_u={u}.pdf", dpi=500)

    plt.show()
    plt.clf()
    plt.close()

    cp.savez_compressed(f"raw_datas/exp0_u={u}", dfs=dfs)
